...
``` r
ggplot(data = mpg) +
  geom_point(mapping = aes(x = displ, y = hwy), color = "blue", size = 3, alpha = .5, shape = 20)
```
...
q + facet_grid(. ~ cyl) # One row four columns by cyl
```

![](Week-4-Visualize-Exercises_files/figure-commonmark/unnamed-chunk-8-1.png)

``` r
q + facet_grid(drv ~ .) # one row four columns by drv
```
...
The different codes creates a graph per variable for the number of
cylinders in the engine vs the variables displ and hwy. With Wrap
creating them all together.
...
``` r
ggplot(data = mpg) +
  geom_bar(aes(x = class, fill = drv), position = position_dodge()) + labs( x = "Class of Vehicle", y = "Number of Vehicles in sample", fill = "Type of Drive") +
  theme_bw()
```
...
ggplot(mpg, aes(x = displ, y = hwy)) + 
  ...
  geom_smooth(color = "black", se = FALSE, method = "lm") + scale_color_viridis_d() 
```
...
Graphs both the scatterplot and smooth lineplot graphs into one plot
then save them as example.jpg image into the file.
